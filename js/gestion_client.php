...
if (isset($_GET['action']))
{
	...
	while ($donnees = $req->fetch())
	{
	    ...
	    $contacttel = $donnees['telephonepersacontacter'];
	...
	if (trim($_GET['action'])=='suppr')
	{
	    ...
	    $req = $bdd->prepare('SELECT * FROM clients, vehicules, clientvehicule WHERE clients.IDCLIENT = clientvehicule.IDCLIENT AND vehicules.IDVEHICULE = clientvehicule.IDVEHICULE AND clients.IDCLIENT = ?');
	    $req->execute(array($id));
	    $count = $req->fetchColumn();
	   // $req->debugDumpParams();
	    if ($count > 0)
	    {
	        header('location: client.php?message=erreur');
	        exit;
	    }
	    else
	    {
	    ...
	    $req->execute(array($id));
	    header('location: client.php?message=ok');
	    exit;
	    }
	}
	else 
	 ...
	 ?>
		...
		<div id="content">
			<div id="content-header">
				...
				<a href="client.php"><i class="icon-user"></i>Client</a>
				<a href="#" class="current"><i class="icon-edit"></i>Modifier client</a>
				</div>
       ...
       <div style="border-radius: 5px;background-color: #f2f2f2;padding: 20px;">	
		  <?php if ((isset($_GET['message'])) && (trim($_GET['message'])=='ok')){?>
              ...
              <?php } ?> 			
		<form class="form-horizontal" method="post" action="actionClient.php" name="basic_validate" id="basic_validate" novalidate="novalidate">
                ...
                <input type="text" name="contact" class="w3-input w3-border" placeholder="Contact"  value = "<?php echo $contact; ?>"/>
                <label class="w3-text-blue"><b>Telephone contact</b></label>
                <input type="text" name="contacttel" class="w3-input w3-border" placeholder="Telephone contact"  value = "<?php echo $contacttel; ?>"/>
                <input type="hidden" name="id" value = "<?php echo $id; ?>"/>
                <br/><br/><input type="submit" class="w3-btn w3-blue" name="modifier" value="Valider">
                <input type="submit" class="w3-btn w3-blue" name="retour" value="Annuler" onclick="location.href='client.php'" >
                </form>
	   </div></div></div></div>					
