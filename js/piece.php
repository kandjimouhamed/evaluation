...
$reponse = $bdd->query('SELECT * FROM piece ORDER BY reference ASC');
...
<div id="content">
  <div id="content-header">
    <div id="breadcrumb"> <a href="index.php" title="Retour" class="tip-bottom"><i class="glyphicon glyphicon-home"></i> Accueil</a> <a href="#" class="current"><i class="glyphicon glyphicon-tasks"></i>Piece</a> <a href="ajouter_piece.php" class=""><i class="glyphicon glyphicon-edit"></i>Nouveau</a></div>
            ...
            <table id="tablemarque" class="table table-bordered data-table table-striped">
             ...
             {  
                echo '<tr class="gradeA">';
	             echo  '<td>'.$i.'</td>'; 
                 echo  '<td>'.getMarque2($donnees['IDMARQUE'],$bdd).'</td>';
                 echo  '<td>'.$donnees['reference'].'</td>'; 
                 echo  '<td>'.$donnees['designation'].'</td>'; 
                 echo  '<td>';
                 //echo '<a href="#"><i class="glyphicon glyphicon-search"></i></a>';
                 echo '<a href="gestion_piece.php?action=edit&id='.$donnees['IDPIECE'].'"><i class="glyphicon glyphicon-edit"></i></a>';
                 if ((isset($_SESSION['profil'])) && ($_SESSION['profil'] == 1))
                 { echo '<a href="supprPiece.php?action=suppr&id='.$donnees['IDPIECE'].'" onclick="return(confirm(\'Etes-vous sur de vouloir supprimer cette entree?\'));"><i class="glyphicon glyphicon-trash"></i></a>'; }
                 echo '</td>'; 
                 echo '</tr>';
                $i++;
              }
