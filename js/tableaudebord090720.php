    ...
    <tr>
		  ...
		  while ($donnees = $reponse7->fetch())
        {
		 ...
		 for ($i = 0; $i < sizeof($tabEtat); $i++)
         {
          ...
          $total = $total + $tot;
          //echo '<td>'.$tot.'</td>';	
		  echo '<td><a href = "tableaudebord.php?etat='.$tabEtat[$i].'&localisation='.$idParc.'&filiale='.$initfiliale.'&direction='.$initdirection.'&client='.$initclient.'&vehicule='.$initvehicule.'">'.$tot.'</a></td>';	  
	     }
		 //echo '<td>'.$total.'</td>';
		 echo '<td><a href = "tableaudebord.php?etat=-1&localisation='.$idParc.'&filiale='.$initfiliale.'&direction='.$initdirection.'&client='.$initclient.'&vehicule='.$initvehicule.'">'.$total.'</a></td>';
		...
		echo '<tr><td>Total</td>';
		echo '<td colspan="'.$col.'" style="text-align:right;"><a href = "tableaudebord.php?etat=-1&localisation=-1&filiale='.$initfiliale.'&direction='.$initdirection.'&client='.$initclient.'&vehicule='.$initvehicule.'">'.$total2.'</a></td></tr>';
		?>
